...
Streak length of 1 means that there are 1 hit and 1 miss in the streak.
Streak length of 0 means that there are 0 hits and 1 miss, i.e. player
had only one shot and it was a miss.
...
This code shows statistic summary for `kobe_streak length`variable:

```{r}
ggplot(kobe_streak, aes(x = length)) +
  geom_histogram() +
  labs(title = "Kobe's streak length histogram",
       x = "Length of streak",
       y = "Count of streaks")

ggplot(kobe_streak, aes(y = length)) +
  geom_boxplot() +
  labs(title = "Kobe's streak lengths boxplot",
       y = "Streak length")

print(summary(kobe_streak$length))
max(kobe_streak$length)
```

Median value gives an idea that Kobe's typical streak length is 0.

His longest streak of baskets is 4, equals to max value from statistic
summary printed above.

From histogram and boxplot I can say that distribution is unimodal and
skewed to the right with most streak lengths are 0 and 1. Outliers
indicate that Kobe's streaks of 3 and 4 baskets are usually rare. The
histogram also shows that Kobe's typical streak length is 0, since most
occurred streaks (mode) has length of 0.
...
The code above shows that 25 flips came up heads from flipping unfair
coin 100 times. Its probability $P(heads) = 25 / 100 = 0.25$ close to
the probablity 0.2 set for "heads" in the code.
...
To reflect a shooting percentage of 45% and simulate 133 shots, we need
to set `prob = c(0.45, 0.55)` and `size = 133` in the `sample` function.
...
Here we can see that hits (H) count is 64 and its probability is
$P(H) = 64 /133 = 0.48$ and it is close to probability 0.45.
...
This code cell computes the streak lengths for simulated shooter:
...
This code creates a histogram and boxplot for simulated shooter's
streak:

```{r}
ggplot(sim_streak, aes(x = length)) +
  geom_histogram() +
  labs(title = "Simulated shooter's streak length histogram",
       x = "Length of streak",
       y = "Count of streaks")

ggplot(sim_streak, aes(y = length)) +
  geom_boxplot() +
  labs(title = "Simulated shooter's streak lengths boxplot",
       y = "Streak length")

print(summary(sim_streak$length))

```

The histogram indicates that most streaks are 0 and 1, and the
distribution is unimodal and skewed to the right. The statistic summary
shows that the median streak length is 1, and the longest streak is 3.
Since median streak length is 1, it is the typical streak length for
simulated shooter.
...
If I run the simulation second time, I would expect its streak
distribution to be somewhat similar to the first one. This is because
each simulated shooter's shot is independent event in the code above,
i.e. previous shot's outcome has no effect on the outcome of the next
shot. Another reason is that probability of hit set to 0.45 and shots
amount set to 133 on each separate simulation run. Shape of distribution
also will be somewhat similar, unimodal and skewed to the right.
...
Let's visualize Kobe's and simulated shooter's histograms:

```{r}
kobes_histogram <- ggplot(kobe_streak, aes(x = length)) +
  geom_histogram() +
  labs(title = "Kobe's streak length histogram",
       x = "Length of streak",
       y = "Count of streaks")

simulation_histogram <- ggplot(sim_streak, aes(x = length)) +
  geom_histogram() +
  labs(title = "Simulated shooter's streak length histogram",
       x = "Length of streak",
       y = "Count of streaks")

kobes_histogram | simulation_histogram
```

Kobe's streaks statistic summary:

```{r}
print(summary(kobe_streak))

```

Simulated shooter's statistic summary:

```{r}
print(summary(sim_streak))
```

From these plots I can see Kobe's and simulated shooter's distributions
of streaks are somewhat similar. Both distributions has majority of
streak lengths 0 or 1. Both distributions are unimodal and right-skewed.
Kobe's longest streak is 4, while simulated shooter's longest streak is
3. Kobe's distribution median is 0, simulated shooter's median is 1.

Let's make mathematical calculations for probabilities of independent
events with formula $$P(A andB) = P(A) * P(B)$$

Probability of streak length of 1 is: $$P(HIT) = 0.45$$

Probability of streak length of 2 is:

$$
P(HIT1andHIT2) = 0.45 * 0.45 = 0.2025
$$

We can see that probability decreases when streak length increases.
Because of that, distribution histograms also showing decrease of streak
counts with longer lengths. And it explains why distributions are
right-skewed.

According to "hot hand" model, probability of making a hit increases
after previous successful hit. And that would make the distribution
histograms show a higher frequency of longer streaks and shape of
distribution would be left-skewed. This is opposite to what we see in
Kobe's and simulated shooter's histograms, where distribuations are
right-skewed because of decreasing probability as I mentioned earlier.

Conclusion:

I can say from above analyses that "hot hand" theory doesn't fit Kobe's
pattern and it appears that Kobe's shots are independent events.
