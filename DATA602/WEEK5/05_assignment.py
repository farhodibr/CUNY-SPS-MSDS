...
def exercise01():
    ...
    df = pd.DataFrame(np.array([25, 3, 42, 9, 5, 11, 8, 2,14, 6, 10, 1]).reshape(3, 4))
...
def exercise05(n):
    ...
    # ------ Place code below here \/ \/ \/ ------
    zeros  = np.zeros(n**2).reshape(n, n)
...
def exercise08():
    ...
    row_count = df.shape[0] # or row_count = len(df)
    avg_sq_ft = df.sq__ft.mean(skipna=True)
    df_zip_95670 = df[df.zip == 95670].copy()
    df_zip_not_95610 = df[df.zip != 95610].copy()
...
def exercise11(n):
    ...
    if n%3 == 0:
        ...
        #print(f'{n} is not divisible by 3. Not able to reshape into {n/3} columns and 3 rows')
        array_reshaped = None
    # ------ Place code above here /\ /\ /\ ------  
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = (1 - np.arange((2*n)**2)%2).reshape(2*n, 2*n)
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df_date = pd.date_range(start='1/1/2010', periods=n )
    df = pd.Series(np.random.randint(0, n +1, size=n), index=df_date)
    s = df.cumsum()

    ax = s.plot(figsize=(12, 7), grid=False)
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = exercise08()[0].iloc[(lambda x: x.index % 5 == 0), [0, 2]].copy()
